...
// ─── Get Event Details ──────────────────────────
exports.eventsRouter.get('/:id', async (req, res, next) => {
    try {
...
// ─── Purchase Ticket ────────────────────────────
exports.eventsRouter.post('/:id/purchase', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const eventId = req.params.id;
        // Get event
        const { data: event } = await supabase_1.supabaseAdmin
            .from('events')
            .select('*')
            .eq('id', eventId)
            .single();
        if (!event)
            throw new error_middleware_1.AppError('Event not found', 404);
        // Check if already has ticket
        const { data: existingTicket } = await supabase_1.supabaseAdmin
            .from('event_tickets')
            .select('id')
            .eq('event_id', eventId)
            ...
            .single();
        if (existingTicket) {
            throw new error_middleware_1.AppError('You already have a ticket for this event', 409);
        }
        // Check capacity
        if (event.max_attendees) {
            ...
            if ((count || 0) >= event.max_attendees) {
                throw new error_middleware_1.AppError('Event is sold out', 409);
            }
        }
        ...
        const { data: ticket, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError('Failed to create ticket', 500);
        // Notification
...
// ─── Get My Tickets ─────────────────────────────
exports.eventsRouter.get('/me/tickets', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const { data, error } = await supabase_1.supabaseAdmin
            .from('event_tickets')
...
exports.eventsRouter.post('/tickets/validate', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const { qr_code } = req.body;
        if (!qr_code)
            throw new error_middleware_1.AppError('QR code is required', 400);
        const { data: ticket, error } = await supabase_1.supabaseAdmin
            .from('event_tickets')
