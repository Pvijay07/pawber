...
exports.slotsRouter.get('/provider/:providerId', async (req, res, next) => {
    try {
        ...
        if (date) {
            ...
            query = query.gte('slot_date', from_date).lte('slot_date', to_date);
        }
        else {
            // Default: next 7 days
...
exports.slotsRouter.post('/:slotId/lock', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (slot.is_blocked)
            throw new error_middleware_1.AppError('Slot is blocked', 400);
        if (slot.booked_count >= slot.capacity)
            throw new error_middleware_1.AppError('Slot is fully booked', 409);
        // Check for existing active locks (excluding expired ones)
        const { data: existingLocks } = await supabase_1.supabaseAdmin
            .from('slot_locks')
            .select('id, user_id')
            .eq('slot_id', slotId)
        ...
        if (totalReserved >= slot.capacity) {
            throw new error_middleware_1.AppError('Slot is currently being booked by someone else. Try again shortly.', 409);
        }
        // Create lock (5 minute expiry)
        const lockExpiry = new Date(Date.now() + 5 * 60 * 1000);
        const { data: lock, error } = await supabase_1.supabaseAdmin
            .from('slot_locks')
            .insert({
            slot_id: slotId,
            user_id: req.user.id,
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError('Failed to lock slot', 500);
        res.json({
            lock,
...
// ─── Release a Slot Lock ────────────────────────
exports.slotsRouter.delete('/:slotId/lock', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        await supabase_1.supabaseAdmin
            .from('slot_locks')
            .delete()
...
exports.slotsRouter.post('/bulk', auth_middleware_1.authenticate, (0, auth_middleware_1.authorize)('provider'), (0, validate_middleware_1.validate)(createSlotsSchema), async (req, res, next) => {
    try {
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .select();
        if (error)
            throw new error_middleware_1.AppError('Failed to create slots', 500);
...
exports.slotsRouter.patch('/:id/block', auth_middleware_1.authenticate, (0, auth_middleware_1.authorize)('provider'), async (req, res, next) => {
    try {
        const { data: provider } = await supabase_1.supabaseAdmin
            .from('providers')
            .select('id')
        ...
        const { data, error } = await supabase_1.supabaseAdmin
            ...
            .single();
        if (error)
            throw new error_middleware_1.AppError('Failed to update slot', 500);
        res.json({ slot: data });
    }
    catch (err) {
        next(err);
...
exports.slotsRouter.get('/me', auth_middleware_1.authenticate, (0, auth_middleware_1.authorize)('provider'), async (req, res, next) => {
    try {
        const { data: provider } = await supabase_1.supabaseAdmin
            .from('providers')
            .select('id')
